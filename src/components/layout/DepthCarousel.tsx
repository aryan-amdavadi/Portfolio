...
export interface DepthCarouselProps {
  ...
  renderOverlay?: (item: DepthCarouselItem, isActive: boolean) => React.ReactNode;
  className?: string;
}
...
const DepthCarousel = ({
  ...
  useEffect(() => {
    ...
    onChange, count, depth, spread, tilt, tiltDirection,
    visibleCards, falloff, blur, duration, ease, loop,
    cardWidth, autoplayDelay
  ]);
  ...
  useEffect(() => {
    // Scroll hijacking removed to respect native page scrolling.
    // Navigation is handled via pointer drag, arrows, or clicks.
  }, [layout, setFocus]);
      ...
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            ...
            />
            {renderOverlay && (
              <div className="depth-carousel__custom-overlay">
                {renderOverlay(item, active === i)}
              </div>
            )}
          </div>
        ))}
